...
if __name__ == '__main__':
    ...
    G1 = kamol.kappa_to_molecule('B(b[1]{a}), B(b[1])')
    print(G1.show())
    maps = SGM.automorphisms(G1)
    print(f'{len(maps)} automorphisms:')
    print_map(maps)
    print(f'{"-"*20}')
    G1 = kamol.kappa_to_molecule('A(a[1],b[2]),A(a[1],b[3]),B(a[2],c[4]),B(a[3],c[5]),C(b[4],x{u}), C(b[5],x{p})')
    G1.show()
    maps = SGM.automorphisms(G1)
    print(f'{len(maps)} automorphisms:')
    print_map(maps)
    ...
    SGM = SiteGraphMatcher()
    G1 = kamol.kappa_to_molecule('APC(pr{p})')
    # G1 = kamol.kappa_to_molecule('A(a[1],b[2]),A(a[1],b[3]),B(a[2],c[4]),B(a[3],c[5]),C(b[4],x{u}[.]),C(b[5],x{p}[.])')
    # G2 = kamol.kappa_to_molecule('A(a[1],b[2]),A(a[1],b[3]),B(a[2],c[4]),B(a[3],c[5]),C(b[5],x{p}[.]),C(b[4],x{u}[.])')
    print(G1.show())
    # print(G2.show())
    maps = SGM.automorphisms(G1)
    print(f'{len(maps)} automorphisms:')
